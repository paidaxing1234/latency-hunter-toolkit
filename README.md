...
[![version](https://img.shields.io/badge/version-0.1.0-3b82f6.svg)](#路线图)
[![skills](https://img.shields.io/badge/skills-2-8b5cf6.svg)](#两个-skill)
...
当前含两个 skill,均按"严重度分级 + 定位到 `文件:行` + 不臆测"的铁律工作。
...
# 2. 安装插件(包含两个 skill + 两个 slash command)
/plugin install latency-hunter
```

装完后,两个 skill 会在你聊到回测/热路径相关话题时**自动触发**,也可以用 slash command 显式调用:
...
重启 Claude Code 后,直接说"帮我审一下这个回测有没有未来函数"或"看看这段热路径为什么慢"即可触发。
...
`v0.1.0` 是地基:两个工程审查 skill。后续按"一人量化全链路工程审查"的思路继续补,**仍然坚持无 alpha、无密钥、无模型**:

- [ ] **`exchange-connector-forge`** —— 交易所连接器审查/脚手架:WebSocket 重连与心跳、限频与权重、订单状态机一致性、`recvWindow`/时间同步、断线重放与幂等。
